...
const cleanSingleFabricAndPanna = (rawFabric, rawPanna) => {
  if (!rawFabric) return { combinedName: '', baseName: '', panna: '58' };
  let str = String(rawFabric).trim().toUpperCase();

  // Extract trailing digits (width numbers) repeated one or more times e.g. "POLLY MAL 38 38" or "FRENCH CREPE 58"
  let extractedPanna = '';
  const pannaMatches = str.match(/(?:\s+(\d+))+\s*$/);
  if (pannaMatches) {
    const digits = pannaMatches[0].trim().split(/\s+/);
    extractedPanna = digits[digits.length - 1]; // take the last valid digit
...
async function run() {
  ...
  console.log('\n--- Cleaning FabricTransactions ---');
  const fabTxs = await FabricTransaction.find({});
  let txFixed = 0;
  for (const tx of fabTxs) {
    if (!tx.fabricQuality) continue;
    const { combinedName, panna } = cleanSingleFabricAndPanna(tx.fabricQuality, tx.panna);
  ...
  console.log(`Cleaned ${txFixed} FabricTransactions.`);
  ...
  for (const jc of jobCards) {
    ...
    const { combinedName, panna } = cleanSingleFabricAndPanna(jc.fabric, jc.panna);
  ...
  console.log(`Cleaned ${jcFixed} JobCards.`);

  console.log('\n--- Cleaning FabricChallans ---');
  const challans = await FabricChallan.find({});
  let fcFixed = 0;
  for (const fc of challans) {
    if (!fc.fabricName) continue;
    const { combinedName, panna } = cleanSingleFabricAndPanna(fc.fabricName, fc.panna);
  ...
  console.log(`Cleaned ${fcFixed} FabricChallans.`);

  console.log('\n--- Cleaning FabricStockAdjustments ---');
  const sas = await FabricStockAdjustment.find({});
  let saFixed = 0;
  for (const sa of sas) {
    if (!sa.fabricQuality) continue;
    const { combinedName, panna } = cleanSingleFabricAndPanna(sa.fabricQuality, sa.panna);
  ...
  console.log(`Cleaned ${saFixed} FabricStockAdjustments.`);

  console.log('\n--- Updating PrintConfig Fabrics Catalog ---');
  const sortedList = Array.from(uniqueFabrics).filter(Boolean).sort();
  console.log('Cleaned Unique Fabrics List:', sortedList);
  ...
  console.log('\n✅ DOUBLE PANNA FABRIC NAMES FIXED PERFECTLY!');
  process.exit(0);
}
