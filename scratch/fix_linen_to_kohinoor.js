...
const cleanLinenToKohinoor = (rawFabric, rawPanna) => {
  ...
  let base = str;
  // Merging LINEN / KOINUR LINEN into KOHINOOR LINEN
...
async function run() {
  ...
  console.log('\n--- Merging LINEN into KOHINOOR LINEN in FabricTransactions ---');
  const fabTxs = await FabricTransaction.find({});
  let txFixed = 0;
  for (const tx of fabTxs) {
    if (!tx.fabricQuality) continue;
    const { combinedName, panna } = cleanLinenToKohinoor(tx.fabricQuality, tx.panna);
  ...
  console.log('\n--- Merging LINEN into KOHINOOR LINEN in JobCards ---');
  const jobCards = await JobCard.find({});
  let jcFixed = 0;
  for (const jc of jobCards) {
    if (!jc.fabric) continue;
    const { combinedName, panna } = cleanLinenToKohinoor(jc.fabric, jc.panna);
  ...
  console.log('\n--- Merging LINEN into KOHINOOR LINEN in FabricChallans ---');
  const challans = await FabricChallan.find({});
  let fcFixed = 0;
  for (const fc of challans) {
    if (!fc.fabricName) continue;
    const { combinedName, panna } = cleanLinenToKohinoor(fc.fabricName, fc.panna);
  ...
  console.log('\n--- Merging LINEN into KOHINOOR LINEN in FabricStockAdjustments ---');
  const sas = await FabricStockAdjustment.find({});
  let saFixed = 0;
  for (const sa of sas) {
    if (!sa.fabricQuality) continue;
    const { combinedName, panna } = cleanLinenToKohinoor(sa.fabricQuality, sa.panna);
  ...
  console.log('\n✅ LINEN & KOINUR LINEN MERGED INTO KOHINOOR LINEN PERFECTLY!');
  process.exit(0);
}
