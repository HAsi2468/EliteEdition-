...
async function run() {
  ...
  const pannaRegex = /^\s*46["']?\s*$/i;

  console.log('\n--- Checking FabricTransactions ---');
  const fabTxs = await FabricTransaction.find({ panna: pannaRegex });
  console.log(`Found ${fabTxs.length} FabricTransactions with panna 46.`);
  ...
  console.log(`Found ${jobCards.length} JobCards with panna 46.`);
  ...
  console.log(`Found ${challans.length} FabricChallans with panna 46.`);
  ...
  for (const cfg of configs) {
    ...
    if (cfg.widths && Array.isArray(cfg.widths)) {
      const updated = cfg.widths.map(w => (String(w).trim().replace(/['"]/g, '') === '46' ? '56' : String(w).trim().replace(/['"]/g, '')));
  ...
  console.log('\n✅ ALL DB PANNA 46 RECORDS UPDATED TO 56');
  process.exit(0);
}
